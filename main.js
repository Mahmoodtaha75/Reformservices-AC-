 // Slideshow Logic
        ...
        function updateCartDisplay() {
            ...
            const visitingCharges = cart.length > 0 ? 219 : 0;
            ...
            // Add visiting charges if cart is not empty
            if (cart.length > 0) {
                const visitingChargesHTML = `
                    <div class="cart-item">
                        <div class="cart-item-details">
                            <div class="cart-item-name">Visiting Charges</div>
                        </div>
                        <div class="cart-item-price">₹${visitingCharges}</div>
                    </div>
                `;

                const mobileVisitingChargesHTML = `
                    <div class="mobile-cart-item">
                        <div class="mobile-cart-item-details">
                            <div class="mobile-cart-item-name">Visiting Charges</div>
                        </div>
                        <div class="mobile-cart-item-price">₹${visitingCharges}</div>
                    </div>
                `;

                cartHTML += visitingChargesHTML;
                mobileCartHTML += mobileVisitingChargesHTML;
            }
            ...
            // Calculate total
            const total = cart.reduce((sum, item) => sum + (item.price * item.qty), 0) + visitingCharges;
        ...
        function sendToWhatsApp() {
            ...
            // Add visiting charges and total
            const visitingCharges = 219;
            const total = cart.reduce((sum, item) => sum + (item.price * item.qty), 0) + visitingCharges;

            servicesText += `\nVisiting Charges: ₹${visitingCharges}\n`;
            servicesText += `Total: ₹${total}`;
